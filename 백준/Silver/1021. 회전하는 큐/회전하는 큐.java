...
// https://velog.io/@gayeong39/백준-1021-회전하는-큐-JAVA
public class Main {
    ...
    // solution(최소 비용을 구하는 메소드) - 파라미터: 뽑으려는 숫자의 개수
    static int solution(int M){
        int cost = 0; //비용(최소)
        // 찾으려는 숫자의 개수만큼 반복하고 찾았을 때 증가한 cost만큼 누적 더하기 실시
        for (int i = 0; i < M; i++) {
            // 찾으려는 숫자의 인덱스 값을 넣어줌
            ...
            // 큐의 사이즈 (저장된 숫자가 짝수 개면)
            if(queue.size() % 2 == 0){
                //인덱스는 0부터 시작하기 때문에 -1을 해줌
                middleIndex = queue.size() / 2 - 1;
            }else{ // 홀수개이면
                // 홀수 개일 경우는 어차피 한쪽이 1개 더 많으니 -1 해주지 x
                middleIndex = queue.size() / 2;
            }

            // 중간 지점이거나 중간지점보다 앞에 있을 경우(2번: 앞에서 뒤로)
